...
ostream& operator<<(ostream& o, const quple& q)
{

    o << " statement: " << q.statement_ << " operator: " << q.op_ << " arg1: " << q.arg1_ << " arg2: " << q.arg2_ << " result: " << q.result_ << " ln: " << q.line_no_ << " cn: " << q.col_no_;
	return o << "\n";
}

string quple_list::get_previous_temp()
{
    if(q_list.size()!=0)
    {   
        auto iter = --q_list.end();
        auto iter2 = --q_list.begin();
        while( iter!=iter2 && (*iter)->result_[0] != 't')--iter;
        if(iter!=iter2)
        return (*iter)->result_;
    }
    return string();
